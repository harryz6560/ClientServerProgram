			...
			for(int i = 0; i < 3; i++){
				...
				if(fds[i].revents & POLLIN){
					...
					if(temp_packet.type == PUT){
						...
						if (jToDelete != -1) {
							send_packet(fifosc[i], ERROR, i, "Object already exits");
						...
						if (jToDelete == -1) {
							send_packet(fifosc[i], ERROR, i+1, "Object not owner");
							printf("Transmitted (src= server) (ERROR: Object not owner) \n");
						}
						else{
...
int main(int argc, char *argv[]) {
	...
	if (strcmp(argv[1], "-s") == 0) do_server();
